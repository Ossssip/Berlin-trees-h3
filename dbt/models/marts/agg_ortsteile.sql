...
tree_admin AS (
    SELECT
        a.ortsteil_code AS area_id,
        t.genus,
        t.species,
        t.source
    FROM trees_coords t
    JOIN {{ ref('stg_ortsteile_detail') }} a ON ST_Within(t.geometry, a.geometry)
),
agg AS (
    SELECT
        area_id,
...
admin_geoms AS (
    SELECT ortsteil_code AS area_id, geometry FROM {{ ref('stg_ortsteile_detail') }}
),
...
SELECT
    ...
    ST_FlipCoordinates(ST_Transform(a.geometry, 'EPSG:25833', 'EPSG:4326'))
                                                                       AS geometry
FROM {{ ref('stg_ortsteile_detail') }} a
JOIN agg b              ON a.ortsteil_code = b.area_id
LEFT JOIN dominant_n d  ON a.ortsteil_code = d.area_id
LEFT JOIN tree_genus_histogram tg ON a.ortsteil_code = tg.area_id
LEFT JOIN forest_histogram fh     ON a.ortsteil_code = fh.area_id
LEFT JOIN forest_totals ft        ON a.ortsteil_code = ft.area_id
